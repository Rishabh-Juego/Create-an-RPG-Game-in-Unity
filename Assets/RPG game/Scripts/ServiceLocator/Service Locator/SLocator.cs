...
{
	public class SLocator : MonoBehaviour
	{
		private static SLocator globalLocator;
		private static Dictionary<Scene, SLocator> SceneContainers = new Dictionary<Scene, SLocator>();
		...
		/// <summary>
		/// Called to set the current <see cref="SLocator"/> for global scope, by default this has the <see cref="MonoBehaviour.DontDestroyOnLoad"/> property set as true
		/// </summary>
		...
		/// <summary>
		/// Called to set the current <see cref="SLocator"/> for the current Scene scope. The service locator has the same scope as whichever scene the current gameObject resides in 
		/// </summary>
		internal void ConfigureForScene()
		{
			...
			if (SceneContainers.TryGetValue(scene, out SLocator attachedLocator))
			{
		...
		/// <summary>
		/// Called to set the current <see cref="SLocator"/> for the current GameObject scope. The service locator has the same scope as whichever GameObject the current ServiceLocator is attached to
		/// </summary>
		internal void ConfigureForGameObject()
		{
			SLocator[] gobScopedSLs = gameObject.GetComponents<SLocator>();
			if (gobScopedSLs.Length != 1 || gobScopedSLs[0] != this)
			{
				Debug.LogError($"One Object cannot have more than one {nameof(SLocator)} attached to it.", gameObject);
				return;
			}
		...
		/// </summary>
		public static SLocator GetSlGlobal
		{
			get
			{
				if (globalLocator != null)
				...
				*/
				...
				GameObject globalContainer = new GameObject(k_globalServiceLocatorName, typeof(SLocator));
		...
		public static SLocator GetSlForSceneOf(MonoBehaviour contextMb)
		{
			...
			if (SceneContainers.TryGetValue(scopeScene, out SLocator sLocator))
			{
				if (sLocator != null)
				{
					return sLocator;
		...
		public static SLocator GetSlForGameObjectOf(MonoBehaviour contextMb)
		{
			SLocator gobServiceLocator = contextMb.GetComponentInParent<SLocator>();
		...
		{ 
			...
			SLocator currScope = this;
			while (TryGetNextServiceLocatorInHierarchy(currScope, out SLocator attachedLocator))
			{
		...
		bool TryGetNextServiceLocatorInHierarchy(SLocator currScope, out SLocator attachedLocator)
		{
		...
		{
			globalLocator = null;
			SceneContainers = new Dictionary<Scene, SLocator>();
